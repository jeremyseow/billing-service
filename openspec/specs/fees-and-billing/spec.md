# fees-and-billing Specification

## Purpose

Provides a deterministic, stateful system to track bills, aggregate fees in multiple currencies with arbitrary decimal precision and FX conversions, manage lifecycles, and support idempotent transaction operations.
...
- **WHEN** a client requests creation of a bill for an account with a start timestamp, end timestamp, and settlement currency
...
The system SHALL support adding a line item to a bill. The system MUST reject adding line items if the bill status is not `OPEN`. The system MUST deduplicate requests using an `idempotency_key`. The system MUST accept amounts formatted as decimal strings to preserve arbitrary precision.

#### Scenario: Adding item to an open bill
- **WHEN** a client adds a line item with a unique `idempotency_key`, a decimal string amount, and a supported currency (USD or GEL) to an open bill
- **THEN** the system SHALL calculate FX rates, record the line item with original and settlement conversion fields, update running `original_totals` and `settlement_total`, and return the updated totals.
...
### Requirement: Multi-Currency Aggregation, Decimal Math, and FX Rate Conversion
The system SHALL maintain running and final totals aggregated by original currency as well as converted grand totals in settlement currency using arbitrary-precision decimal math (`shopspring/decimal`). The system MUST audit original transaction amount/currency, FX rate, and settlement amount/currency for each line item.

#### Scenario: Running totals with FX rate conversion and mixed positive and negative items
- **WHEN** line items of positive and negative decimal amounts in USD and GEL are added to an open bill with settlement currency USD
- **THEN** the system SHALL convert GEL items to USD using current FX exchange rates, aggregate original currency balances in `original_totals`, compute the converted grand total in `settlement_total`, and persist all FX audit metadata to PostgreSQL.
...
The system SHALL support querying a bill's current details, including its metadata, original currency totals, settlement grand total, and all associated line items with full FX rate audit metadata.

#### Scenario: Retrieving detailed bill details
- **WHEN** a client queries a bill by its ID
- **THEN** the system SHALL query PostgreSQL directly and return the bill metadata, line items with original/settlement FX fields, original totals breakdown, and settlement total.
...
- **WHEN** billing events (such as workflow start, line item insert, manual close, rollover, or termination) are executed
...
The system SHALL enforce that an account can have at most one active open bill at any time. This constraint MUST be validated at the API layer and enforced at the database layer via a partial unique index (`idx_unique_open_bill_per_account`).
...
The system SHALL support terminating a billing period permanently via `POST /bills/:id/terminate`. This operation MUST finalize the current bill with a `TERMINATED` status, record the accumulated totals, and prevent the auto-spawning of any consecutive billing cycle.
